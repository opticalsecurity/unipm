...
describe("Package Manager Detection", () => {
    ...
    test("should detect from package.json packageManager field", async () => {
        mockExists.mockImplementation((path: string) => Promise.resolve(path === "package.json"));
    ...
    test("should detect from bun.lock", async () => {
        mockExists.mockImplementation((path: string) => Promise.resolve(path === "package.json" || path === "bun.lock"));
    ...
    test("should detect from package-lock.json", async () => {
        mockExists.mockImplementation((path: string) => Promise.resolve(path === "package.json" || path === "package-lock.json"));
        mockJson.mockImplementation(() => Promise.resolve({}));
    ...
    test("should handle error during detection", async () => {
        mockExists.mockImplementation(() => { throw new Error("FS Error"); });
    ...
    test("should detect from pnpm-lock.yaml", async () => {
        mockExists.mockImplementation((path: string) => Promise.resolve(path === "package.json" || path === "pnpm-lock.yaml"));
        mockJson.mockImplementation(() => Promise.resolve({}));

        mockSpawn.mockImplementation((args: string[]) => {
            if (args[0] === "pnpm") {
                return { exited: Promise.resolve(0), stdout: new Response("8.0.0").body };
            }
            return { exited: Promise.resolve(1), stdout: new Response("").body };
        });
